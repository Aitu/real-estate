...
const handler = NextAuth({
  ...
  callbacks: {
    async signIn({ account, profile }) {
      ...
      if (!existing) {
        ...
        const newUser: NewUser = {
          ...
          avatarUrl: 'picture' in profile ? (profile.picture as string | null | undefined) ?? null : null,
          role: 'owner'
        };
    ...
    async jwt({ token, user }) {
      if (user) {
        token.id = user.id;
        token.role = (user as Record<string, unknown>).role ?? 'member';
        token.locale = (user as Record<string, unknown>).locale ?? 'en';
      } else if (token.email) {
